...
in normalised packet-latent space; the question the run answers is whether that
single bottleneck vector beats the "learn nothing" baselines in baseline_mses
(predict the global mean latent / the per-position mean latent).
...
TRAIN_LATENT_CACHE = "data_artefacts/IIoTset-Ferrag/split/latents_EdgeIIoT_E0/train"
VAL_LATENT_CACHE = "data_artefacts/IIoTset-Ferrag/split/latents_EdgeIIoT_E0/val"
...
RETRIEVAL_VAL_BATCHES = 8      # retrieval_accuracy is O(n^2); cap how much of val it sees
...
val_latents, val_flow_offsets, val_meta = load_latent_cache(VAL_LATENT_CACHE, PACKET_AE_CKPT)
...
def evaluate():
    ...
    retrieval_sum = retrieval_n = 0.0
    retrieval_batches = 0
    base_global = base_position = 0.0

    for index, windows in enumerate(val_batches):
        ...
        if index < RETRIEVAL_VAL_BATCHES:
    ...
    n_retrieval = max(1, retrieval_batches)
    model.train()
    return {
        ...
        "val_retrieval": retrieval_sum / n_retrieval,
        "val_retrieval_n": retrieval_n / n_retrieval,
        "baseline_global": base_global / n_retrieval,
        "baseline_per_position": base_position / n_retrieval,
    }
...
for epoch in range(start_epoch, Epochs):
    ...
    logger.info(f"Val Retrieval Accuracy: {val_metrics['val_retrieval']:.4f} "
                f"(n={val_metrics['val_retrieval_n']:.0f})")
...
if history:
    ...
    axes[1].plot(epochs_axis, [r["val_retrieval"] for r in history], label="val retrieval acc")
    axes[1].plot(epochs_axis, [r["val_length_acc"] for r in history], label="val length acc")
    axes[1].set_xlabel("epoch"); axes[1].set_ylabel("accuracy")
    axes[1].set_ylim(0, 1); axes[1].set_title("Retrieval / length accuracy"); axes[1].legend()
